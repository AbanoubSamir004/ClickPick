...
function ProductSearchResults() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        let endpoint = '';
        if (location.pathname.includes('/Products/search')) {
          endpoint = `http://127.0.0.1:8000/api/search/${query}/`;
        } else if (location.pathname.includes('/Products/subcategory')) {
          endpoint = `http://127.0.0.1:8000/api/subcategory/${subcategory}`;
        }
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      try {
        let br_endpoint;
        if (location.pathname.includes('/Products/search')) {
          br_endpoint = `http://127.0.0.1:8000/api/search/${query}/brands/`;
        } else {
          br_endpoint = `http://127.0.0.1:8000/api/subcategory/${subcategory}/brands/`;
        }
        const response = await axios.get(br_endpoint);
  ...
  function handleViewMoreClick() {
    if (filteredProducts.length === 0) {
      // No products to fetch, so return without updating state
      return;
    }
    ...
    return (
      <div className="category-breadcrumb">
        {props.ProductCategory && (
          <>
            <button className="breadcrumb-button" type="button" onClick={handleCategoryClick}>
              {props.ProductCategory}
            </button>
            <span>/</span>
          </>
        )}
        <button className="breadcrumb-button" type="button" onClick={handleSubcategoryClick}>
  ...
  return (
    <div className="product-search-results">
      ...
      <CategoryBreadcrumb  
          ...
          ProductSubCategory={productsData[1]?.ProductSubCategory}
          />
              ...
              <div className="product-actions">
                ...
                <p className='ratingcount'>
                  {product.ProductRatings !== '' ? (
                    ...
                    <>
                      No Rating{' '}
                      {product.ProductRatingCount.length === '()' ? '(0)' : `(${product.ProductRatingCount})`}
                    </>
                  )}
                </p>
